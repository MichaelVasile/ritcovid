...
def get_statistics():
    ...
    soup = BeautifulSoup(page.content, 'html.parser')
    all_students = str(soup.find('div', attrs = {'class': 'statistic-12481'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    all_staff = str(soup.find('div', attrs = {'class': 'statistic-12484'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    new_students = str(soup.find('div', attrs = {'class': 'statistic-12202'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    new_staff = str(soup.find('div', attrs = {'class': 'statistic-12205'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    campus_quarantine = str(soup.find('div', attrs = {'class': 'statistic-12190'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    offcampus_quarantine = str(soup.find('div', attrs = {'class': 'statistic-12193'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    campus_isolated = str(soup.find('div', attrs = {'class': 'statistic-12226'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    offcampus_isolated = str(soup.find('div', attrs = {'class': 'statistic-12229'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    isolation_beds = str(soup.find('div', attrs = {'class': 'statistic-12214'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip())
    last_updated = str(soup.find('strong').text)
    new_case_stat = str(soup.find('p', attrs = {'class': 'h2'}).text)
    tests_administered = str(soup.find('div', attrs = {'class': 'statistic-12829'}).find_all("p", attrs = {'class': 'card-header'})[0].text.strip().replace("*"," "))


    return all_students, all_staff, new_students, new_staff, campus_quarantine, offcampus_quarantine, campus_isolated, offcampus_isolated, isolation_beds, last_updated, new_case_stat, tests_administered
...
async def stats(ctx):
    ...
    embed.add_field(name="All Confirmed Cases", value=(statistics[0] + " student(s), " + statistics[1] + " employee(s)"), inline=False)
    embed.add_field(name=statistics[10], value=(statistics[2] + " student(s), " + statistics[3] + " employee(s)"), inline=False)
    embed.add_field(name="Tests Administered On Campus", value=(statistics[11]), inline=False)
    embed.add_field(name="Students Quarantined", value=(statistics[4] + " on campus, " + statistics[5] + " off campus"), inline=False)
    embed.add_field(name="Students Isolated", value=(statistics[6] + " on campus, " + statistics[7] + " off campus"), inline=False)
...
async def help(ctx):
    ...
    embed.add_field(name=".botinfo", value="Displays bot information, including version number and uptime.", inline=False)
